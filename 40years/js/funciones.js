...
anclas=['#home', '#regulation', '#areas', '#timeline', '#milestones', '#directors', '#hightlights', '#testimonies', '#facts', '#looking'];
secciones=['#section0', '#section1', '#section2', '#section3', '#section4', '#section5', '#section6', '#section7', '#section8', '#section9'];	
 window.location.hash = target;	
	...
	for(i=0;i<anclas.length;i++){
		...
		$target = $(target);
	    $("nav.menuMvl #menu").show();

	    $('html, body').stop(true).animate({'scrollTop': $target.offset().top}, 800*(parseInt(1+'.'+(i+5))), 'swing', function () {
	    });
	});
};
	  ...
	  if ($(window).width()<=990){
		$(document).ready(function(){
			jQuery('#botonMenu').click(function () {
			  jQuery('#menu').show();
				if ( jQuery('#menu').hasClass("expand")) {
					jQuery('#menu').removeClass("expand").addClass("is-expanded");
				} else if (jQuery('#menu').hasClass("is-expanded")) {
					jQuery('#menu').removeClass("is-expanded").addClass("expand");
				}
			  });
...
var pulsadoShare=false; 
		...
		$('div.share a').click(function () { 		
			...
			$(this).parent().parent().addClass('coverToolsActive');
			pulsadoShare=true;
			}else{
			$(this).parent().removeClass('activoShare');
			$(this).parent().parent().removeClass('coverToolsActive');
			pulsadoShare=false; 
			};
		}); 
...
var movimiento=0;
var pag=1;
var totalpag=Math.round($(".factsItem").width()/$('.containerFacts').width());
...
$.moveWork=function(dcontainer,dmenu){
	...
	$(".slideMenuWorks").animate({marginLeft:dmenu},1000);	
  	...
  	//alert($(".horizontal").scrollLeft() + " px");
...
$(document).ready(function() {	
         ...
         if ($(window).width()<=800){
			...
			$.initWork('0.6em','0em');
				...
				$.moveWork('0.6em','0em');
				$.transWork(1);	
				$(document).scrollTop( $("#section4").offset().top );  
				});
...
$.nombreRedesPie=function(){
   $('.redesSociales ul li a').html('&nbsp');

};

//añadir scroll horizontal a milestones cuando es mvl o tablet
jQuery(document).ready(function(){
  ...
  if( isMobile.any() ) {
    jQuery('.wraperWork').addClass('horizontal');
  };
