...
* ---------------------------------
*
* time drush -r /var/www/ef scr sites/all/_docs/php-scripts/migration_data_processings/fix-efpages2-file-links.php
* the script fixes all migrated pages internal file references to the new ones
*/
	
	$result = db_query("select page_id, link, replace_with from efpages_links2 where category = 'file' group by page_id, link order by page_id");
	$count = 0;
	...
	foreach ($result as $row) {
		...
		if($page_id != $row->page_id){
			...
			if($page_id!=-1){ // previous 
				...
				if(true){
					$num_updated = db_update('efpages2') -> fields(array(		
			...
			$page_result = db_query("SELECT page_content_text FROM {efpages2} where page_id = ". $page_id );
