...
* ---------------------------------
*
* time drush -r /var/www/ef scr sites/all/_docs/php-scripts/migration_data_processings/fix-efpages-links.php
* the script fixes all migrated pages links based on the related database tables in which the correction has bee performed and the new pages 
* the script applies the corrections
*/
	
	$result = db_query("select page_id, link, replace_with from efpages_links where category = 'webpage' group by page_id, link order by page_id");
	$count = 0;
