...
function _migrate($nodeType,$action,$from,$to) {
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '109' and lang='en' and page_name not like '%\___.htm' order by page_id asc");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '109' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  //die("select * from efpages where type_id = '105' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $translationPageIds = db_query("select * from efpages where type_id = '109' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $row++;
...
function processNode($node, $data, $translations=null) {
  ...
  if (shouldImportValue($data->page_content_title)) {
	...
	$node->field_ef_theme[LANGUAGE_NONE]=$themesIds;*/
		...
		$findme   = 'GMT';
		$deadlineString = substr($data->page_content_text,strpos($data->page_content_text, $findme),150);
		$deadline='';
		if(preg_match('/([a-z]+[, .]+)?(\d.+?)\D*?$/m', $deadlineString, $match))
		{
			$date = '';
		...
		if(!empty($deadline)){
			$node->field_ef_application_deadline[LANGUAGE_NONE][0]=array('value'=>$deadline);
		}
		
		$explodedReference=explode(".",$data->page_name);
		
		$ref=strtoupper($explodedReference[0]);
		
		$referenceNumber=substr($ref,0,2)."/".substr($ref,2,2)."/".substr($ref,4,2);
		echo $referenceNumber;
		
		$node->field_ef_reference_number[LANGUAGE_NONE][0]=array('value'=>$referenceNumber);
...
cache_clear_all();
_migrate("ef_vacancy", $action,$from,$to);
