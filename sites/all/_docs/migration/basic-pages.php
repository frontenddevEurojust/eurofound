...
function _migrate($nodeType,$action,$from,$to) {
	  if ($action=='del') {
		$results = db_query("SELECT distinct(node_id) from efpages where type_id = '110'");
		foreach ($results as $data) {
			if(!empty($data->node_id)){
				echo $data->node_id."<br/>";
				node_delete($data->node_id);
				$query = db_update('efpages')->condition('node_id', $data->node_id)->fields(array('migrated'=>0,'node_id'=>0))->execute();
				sleep(1);
			}
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '110' and lang='en' order by page_id asc limit $from, $to");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '110' and lang!='en' and path='$data->path' and page_name like '%".$pagesId[0]."\_%'");
		  $translationPageIds = db_query("select * from efpages where type_id = '110' and path='$data->path' and lang!='en'and page_name like '%".$pagesId[0]."\_%'");
		  $row++;
...
function processNode($node, $data, $translations=null) {
  ...
  if (shouldImportValue($data->page_content_title)) {
	...
	$node->field_ef_theme[LANGUAGE_NONE]=$themesIds;*/
		...
		$node->path = array('alias' => $nodeBasePath.$nodePageName, 'pathauto' => FALSE);
