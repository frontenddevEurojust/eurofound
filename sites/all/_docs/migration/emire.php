...
function _migrate($nodeType,$action,$from,$to) {
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '300' order by page_id asc limit $from, $to");
...
function processNode($node, $data, $translations=null) {
  ...
  if (shouldImportValue($data->page_content_title)) {
	...
	$country = $countriesToIsoCodes[ucwords(strtolower(trim($countryName)))];
	if ($country) {
	  $node->field_ef_country[LANGUAGE_NONE][0] = array('iso2'=>$country);
	} else {
		  error("missing iso code for country '$country'\n");
	}
...
cache_clear_all();
_migrate("ef_emire_dictionary", $action,$from,$to);
