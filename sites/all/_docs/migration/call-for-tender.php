...
function _migrate($nodeType,$action,$from,$to) {
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '111' and lang='en' and page_name not like '%\___.htm' order by page_id asc");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '111' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  //die("select * from efpages where type_id = '105' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $translationPageIds = db_query("select * from efpages where type_id = '111' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $row++;
...
function _createDrupalNode($nodeType, $legacyId) {
  ...
  $node->created = time();
  //$node->changed  = time();
  //$node->timestamp = time();  // default fallback - otherwise it will be explicitely set in processNode
...
function processNode($node, $data, $translations=null) {
  ...
  if (shouldImportValue($data->page_content_title)) {
	...
	$statusId=getTermIdFromTermName('Closed','ef_procurement_statuses');
	$statusesId[]['tid']=$statusId;
	$node->field_ef_procurement_status[LANGUAGE_NONE]=$statusesId;
		...
		$publicationDate='';
		if(preg_match('/([a-z]+[, .]+)?(\d.+?)\D*?$/m', $data->page_content_title, $match))
		{
			$date = '';
			...
			{
				$publicationDate= date("Y-m-d",$date); 
			}
		}
		else
		{
		...
		if(!empty($publicationDate)){
			$node->field_ef_tender_publication_date[LANGUAGE_NONE][0]=array('value'=>$publicationDate);
		}
...
cache_clear_all();
_migrate("ef_call_for_tender", $action,$from,$to);
