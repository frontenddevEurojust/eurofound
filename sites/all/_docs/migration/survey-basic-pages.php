...
function _migrate($nodeType,$action,$from,$to) {
	  if ($action=='del') {
		$results = db_query("SELECT distinct(node_id) from efpages where type_id = '108'");
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("select * from efpages where type_id='108' and page_name not like '%\___.htm' order by page_id asc limit $from, $to");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '108' and path='$data->path' and page_name like '%".$pagesId[0]."\_%'");
		  $translationPageIds = db_query("select * from efpages where type_id = '108' and path='$data->path' and page_name like '%".$pagesId[0]."\_%'");
		  $row++;
