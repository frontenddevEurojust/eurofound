...
function _migrate($nodeType,$action,$from,$to) {
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '105' and lang='en' and page_name not like '%\___.htm' order by page_id asc limit $from, $to");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '105' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  //die("select * from efpages where type_id = '105' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $translationPageIds = db_query("select * from efpages where type_id = '105' and page_name like '".$pagesId[0]."%\___.htm%' and path='$data->path'");
		  $row++;
...
cache_clear_all();
_migrate("ef_news", $action,$from,$to);
