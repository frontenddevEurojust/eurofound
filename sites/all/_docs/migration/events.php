...
function _migrate($nodeType,$action,$from,$to) {
	  ...
	  $result = "<table><tr><th>No.</th><th>Title</th><th>Url</th><th>Old url</th></tr>";
		...
		$results = db_query("SELECT * from efpages where type_id = '106' and lang='en' order by page_id asc limit $from, $to");
		
		foreach ($results as $index=>$data) {
		  $pagesId = explode(".",  $data->page_name);
		  $translations = db_query("select * from efpages where type_id = '106' and lang!='en'and page_name like '%".$pagesId[0]."\_%'");
		  $translationPageIds = db_query("select * from efpages where type_id = '106' and lang!='en'and page_name like '%".$pagesId[0]."\_%'");
		  $row++;
...
function parseEventMetadata($data){
	if($data){
		$html = str_get_html($data);
		$eventInforation=$html->find("h2",0)->plaintext;
		/*foreach($html->find("div[class=leftdivpub] table tr") as $e) {
			$label=$e->find("td",0)->plaintext;
...
function processNode($node, $data, $translations=null) {
  ...
  if (shouldImportValue($data->page_content_title)) {
	...
	$node->field_ef_theme[LANGUAGE_NONE]=$themesIds;
	  ...
	  $dates = db_query("SELECT * from efpages_event_dates where page_id = '$page_id'")->fetchObject();
	  $node->field_ef_event_start_date[LANGUAGE_NONE][0]=array('value'=>$dates->startd);
	  $node->field_ef_event_end_date[LANGUAGE_NONE][0]=array('value'=>$dates->endd);
		...
		//$node->path['alias'] = $nodeBasePath.$nodePageName;
		//die($nodeBasePath.$nodePageName);
		$eventInformation=parseEventMetadata($data->page_content_text);
...
cache_clear_all();
_migrate("ef_event", $action,$from,$to);
