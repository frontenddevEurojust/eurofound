...
function zurb_foundation_preprocess_node(&$variables) {
  ...
  //Add opengraph image
  $img = array();
  if (($variables['type'] == 'article') || ($variables['type'] == 'ef_event')) {
    $img = field_get_items('node', $variables['node'], 'field_image');
  } else if (($variables['type'] == 'page') || ($variables['type'] == 'board_member_page') || ($variables['type'] == 'ef_report') || ($variables['type'] == 'ef_input_to_erm') || ($variables['type'] == 'ef_news') || ($variables['type'] == 'ef_spotlight_entry') || ($variables['type'] == 'ef_video')) {
    $img = field_get_items('node', $variables['node'], 'field_ef_main_image');
  } else if ($variables['type'] == 'timeline_date') {
    $img = field_get_items('node', $variables['node'], 'field_timeline_media_image');
  } else if ($variables['type'] == 'ef_photo_gallery') {
...
function zurb_foundation_preprocess_page(&$variables) {
  ...
  //Add opengraph image
  $img_url = $GLOBALS['base_url'] . '/sites/all/themes/effoundationtheme/logo_facebook.jpg';
  $og_image = array(
	'#tag'  => 'meta',
        '#attributes' => array(
	   'property'  => 'og:image',
