...
/* CSS SPRITE , discussed the function , later redefine onclick */
...
jQuery(window).load(function(){
  jQuery('a.dd-option').click(function(){
    var path = window.location.pathname;
    var url = window.location.href;
    var language = jQuery(' input', jQuery(this)).attr("value");
    var difference = 0;
    if (language == 'en'){
      if ((path.charAt(0) == '/' &&  path.charAt(3) == '/') || path.length == 3){
        path = path.substr(3, path.length);
        difference += 3;
      }
    }else{
      if ((path.charAt(0) == '/' &&  path.charAt(3) == '/') || path.length == 3){
        path = '/' + language + path.substr(3, path.length);
      }else{
        path = '/' + language + path;
        difference -= 3;
      }
    ...
    if(document.location.hash.length > 0){
       url= originURL + path + document.location.hash;
     }else if(document.location.search.length > 0){
      url= originURL + path + document.location.search;
     }else{
        url= originURL + path;
     }
