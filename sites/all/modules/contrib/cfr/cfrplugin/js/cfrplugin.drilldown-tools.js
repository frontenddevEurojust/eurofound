...
(function($) {
  ...
  function StoragePerInterface() {
    ...
    $(window).bind('storage', function(jqEvent){
      ...
      if ('cfrplugin.' !== event.key.substr(0, 10)) {
        return;
      }
    ...
    this.set = function(iface, values) {
      localStorage.setItem('cfrplugin.' + iface, JSON.stringify(values));
      update(iface, values);
    };

    this.get = function(iface) {
      return JSON.parse(localStorage.getItem('cfrplugin.' + iface));
    };
  ...
  Drupal.behaviors.cfrapiDrilldownTools = {
    attach: function (context, settings) {

      $('select.cfr-drilldown-select', context).each(function() {
        var $select = $(this);
        var $selectWrapper = $select.parent();
        var $drilldown = $selectWrapper.parent();
        var iface = $drilldown.attr('data:cfrplugin_interface');

        var $tools = $('> .cfrplugin-tools', $drilldown);
        if (0 === $tools.length) {
          return;
        }
        ...
        $('.cfrplugin-paste', $tools).each(function() {
          ...
          $paste.click(function(){
            ...
            var $options = $('> .cfrapi-depending-element-container', $drilldown);
            $options.empty();
