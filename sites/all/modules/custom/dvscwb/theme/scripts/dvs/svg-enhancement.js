//kp//eworx//gr// EWORX S.A. 2013-2014 // derived from DVT
...
function updateSVG(){
	...
	jQuery.ajax({
		...
		success: function(response) {
			...
			/*
			var gradiendDataImage = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAcAAAABCAYAAAASC7TOAAAABmJLR0QA/wD/AP+gvaeTAAAAB3RJTUUH3QkKEBgwuD0swQAAAChJREFUCJkBHQDi/wHE2M//5ezpAOXs6gDm7eoA5+PlAOfk5QDo5OYALnwTr+5RWVMAAAAASUVORK5CYII=";


			if(true){
				//this messes the gradiend if existing in the visualizations in i.e.9 - so we can get it an reapply it after the optimization
			...
			*/
			$svgContainer.html(response);//.find("symbol").css("overflow", "visible"); // CAIRO FIX

			//$svgContainer.find("image").attr("xlink:href", gradiendDataImage);
...
var stopVirateLoading = false;
function removeLoading(){
	stopVirateLoading = true;
	jQuery("#loadingSVG").fadeTo( "slow" , 0, function() {
		jQuery(this).remove();
	});
}

function virateLoading(){
	if(!stopVirateLoading)
	jQuery("#loadingSVG").each(function(){
		if(!stopVirateLoading)
...
function getCountryMouseOverHTMLTooltip(country){
	...
	function makePathInteracrive(){
		...
		var elementQuery = "svg path";

		var $interactiveElements = jQuery(elementQuery);


		jQuery(elementQuery).mousemove(function(e) {
			var $this = jQuery(this);

			jQuery( ".style7").unbind( "mouseenter" );
			...
			if($this.css("fill-opacity")=="0.2" || $this.css("fill-opacity")=="0.20000000298023224" )
				return;
			...
			var dAttr = $this.attr("d").substring(0, 35);
			...
			if(plot=="euMap"){
				...
				Tipped.create(
					...
					function(element) {
						...
						if(jQuery("#edit-variables-wrapper .form-type-bef-checkbox input:checked").length==1){
							counter--;
						}
			...
			$this.mouseout(function() {
				...
				var $tooltip = jQuery('div.t_Tooltip.t_visible');
				var isHovered = $tooltip.is(':hover');
				if (!isHovered){
					$tooltip.removeClass('t_visible');
					$tooltip.addClass('t_hidden');
			...
			if(resolveCountries)
				$this.click(function(e) {
					...
					for(var i=0;i<$elementsOfSameStyle.length;i++){
						patterns+= "\n" + currentMouseOverCountry + "\t" + jQuery($elementsOfSameStyle[i]).attr("d").substring(0, 35);
					}
					alert(patterns);
				});
...
*/
	...
	for(i=0;i<countryToPathD.length;i++){
			...
			if(countryToPathD[i][0]=="SE" || countryToPathD[i][0]=="NO" || countryToPathD[i][0]=="PT" || countryToPathD[i][0]=="ES" || countryToPathD[i][0]=="IE"){
