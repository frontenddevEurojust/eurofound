(function ($) {
  ...
  var createModalityFilter = function(data, settingsData){
	...
	{
		var alphaSort = [translatedValue(dataFile, 'optimism_sortOptionDefault'), translatedValue(dataFile, 'optimism_sortOption1'), translatedValue(dataFile, 'optimism_sortOption2')];

		var select = d3.select('body .chart-filters').append('select').property('id', 'sort-filter').property('name', 'sort');

		var options = select.selectAll('option').data(alphaSort).enter().append('option')
			.text(function (d, i) { return d; }).property('value',function(d, i){ return i; });
  ...
  var calculateMaxValue = function (data){
	...
	{	  
		$('.chart-filters').append('<label for="modality-filter" class="label-group-optimism">' + translatedValue(dataFile, 'LabelGroup') + '</label>');
	...
	var translateData = function(dataFile, csv)
	{
		var data = csv.map(function(row)
		{
			...
			row.modalityValue = translatedValue(dataFile, 'optimism_filter_group' + row.modalityCode);
			return row;
		});
		return data;
	}
    ...
    d3.selectAll(".y-axis .tick text")
      ...
      .attr("class", function(d) {  
		...
		axisLines = xAxisGroup.selectAll("path").data(x.ticks(10)).enter().append("path")
			.attr("class", "grid-line").attr("stroke-opacity", "0").attr("d", axisLinePath)
			.transition().duration(750).attr("stroke-opacity", "1");
	...
	$(document).ready(function()
	{
		...
		d3.queue()
			...
			.defer(d3.csv, '/sites/default/files/ejm/data/en/optimism/optimism_en.csv')
			.await(function(error, settingsFile, dataFile, csv)
		{

			if (csv === null)
			{
				console.log('Requested csv at /sites/default/files/ejm/data/en/optimism/optimism_en.csv was not found.');
			}
        ...
        if($(window).width()>=768){
		  ...
		  var classToPos = {
			...
			var legendLabels = [
				{label: translatedValue(dataFile, 'optimism_legend1'), class: "lollipop-start"}, 
				{label: translatedValue(dataFile, 'optimism_legend2'), class: "lollipop-end"},
			];
