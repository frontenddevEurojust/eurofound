(function ($)
{
	...
	{
		var alphaSort = [translatedValue(dataFile, 'work-life-balance_2_sortOptionDefault'), translatedValue(dataFile, 'work-life-balance_2_sortOption1')];

		var select = d3.select('body .chart-filters').append('select').property('id', 'sort-filter').property('name', 'sort');

		var options = select.selectAll('option').data(alphaSort).enter()
		.append('option').text(function (d, i) { return d; }).property('value',function(d, i){ return i; });
	...
	var calculateMinValue = function (data)
	{
		...
		data.forEach(function(row, index)
		{
			...
			minValue = Math.min(minValue, row.dot1);
		});
		return minValue;
	}
	...
	var lollipopLinePath = function(d)
	{
		...
		return lineGenerator([[0, y(d.countryName) + (y.bandwidth() / 2) ], [x(Math.round(d.dot1)), y(d.countryName) + (y.bandwidth() / 2)]]);
	};
	...
	var calculateMaxValue = function (data)
	{
		...
		var result = data.forEach(function(row, index)
		{
			...
			maxValue = Math.max(maxValue, row.dot1);
		});
		return maxValue;
	}
	...
	var translateData = function(dataFile, csv)
	{
		var data = csv.map(function(row)
		{
			...
			row.modalityValue = translatedValue(dataFile, 'work-life-balance_2_filter_data' + row.modalityCode);
			row.subgroupValue = translatedValue(dataFile, 'work-life-balance_2_filter_group' + row.subgroupCode);
			return row;
		});
		return data;
	}
	...
	function updateGraph()
	{
		...
		var customLimits = customSettings(settingsData, 'work-life-2', modalityCode);
		...
		d3.select("body .chart-wrapper svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
		d3.select("body .chart-wrapper svg > g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");


		d3.selectAll("g.tick").remove();

		y = d3.scaleBand().domain(filteredData.map(function(d) { return d.countryName })).range([0, height]).padding(padding);
    ...
    d3.selectAll(".y-axis .tick text")
      ...
      .attr("class", function(d) {  
		...
		axisLines = xAxisGroup.selectAll("path").data(x.ticks(10)).enter().append("path").attr("class", "grid-line")
			.attr("stroke-opacity", "0").attr("d", axisLinePath).transition().duration(750).attr("stroke-opacity", "1"); 
      ...
      .attr("class", function(d) {  
        if(d.highlight == 1){
          ...
          return 'lollipop-start';
			...
			/*var endCircles = lollipops.select("circle.lollipop-end").data(filteredData).transition().duration(transitionD).attr("cx", function(d) 
			{
	...
	$(document).ready(function()
	{
		...
		d3.queue()
			...
			.defer(d3.csv, '/sites/default/files/ejm/data/en/balance-care/balance-care_en.csv')
			.await(function(error, settingsFile, dataFile, csv)
		{
			if (csv === null)
			{
				console.log('Requested csv at "/sites/default/files/ejm/data/en/balance-care/balance-care_en.csv" was not found.');
			}
			...
			var legendLabels = [
				{label: translatedValue(dataFile, 'work-life-balance_2_legend1'), class: "lollipop-start"}, 
			];
			...
			x = d3.scaleLinear().domain([domainMin, domainMax]).range([0, width]).nice();
	    ...
	    d3.selectAll(".y-axis .tick text")
	      ...
	      .attr("class", function(d) {  
			...
			var axisLines = xAxisGroup.selectAll("path").data(x.ticks(10)).enter().append("path")
				.attr("class", "grid-line").attr("d", axisLinePath);
					
			lollipopsGroup = svg.append("g").attr("class", "lollipops");


			lollipops = lollipopsGroup.selectAll("g").data(filteredData).enter().append("g").attr("class", "lollipop");
